...
public class SituationDetailConfig : IEntityTypeConfiguration<SituationDetail>
{
    public void Configure(EntityTypeBuilder<SituationDetail> builder)
    {
        ...
        builder.HasOne(x => x.Situation).WithMany(x => x.SituationDetails).HasForeignKey(x => x.SituationId).OnDelete(DeleteBehavior.ClientCascade);
        builder.HasOne(x => x.Detail).WithMany(x => x.SituationDetails).HasForeignKey(x => x.DetailId).OnDelete(DeleteBehavior.ClientCascade);
    }
}
