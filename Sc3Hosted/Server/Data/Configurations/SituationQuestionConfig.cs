...
public class SituationQuestionConfig : IEntityTypeConfiguration<SituationQuestion>
{
    public void Configure(EntityTypeBuilder<SituationQuestion> builder)
    {
        ...
        builder.HasOne(x => x.Question).WithMany(x => x.SituationQuestions).HasForeignKey(x => x.QuestionId);
        builder.HasOne(x => x.Situation).WithMany(x => x.SituationQuestions).HasForeignKey(x => x.SituationId);
    }
}
