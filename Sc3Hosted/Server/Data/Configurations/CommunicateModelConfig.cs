...
public class CommunicateModelConfig : IEntityTypeConfiguration<CommunicateModel>
{
    public void Configure(EntityTypeBuilder<CommunicateModel> builder)
    {
        ...
        builder.HasOne(x=>x.Model).WithMany(x=>x.CommunicateModels).HasForeignKey(x=>x.ModelId).OnDelete(DeleteBehavior.ClientCascade);
        builder.HasOne(x=>x.Communicate).WithMany(x=>x.CommunicateModels).HasForeignKey(x=>x.CommunicateId).OnDelete(DeleteBehavior.ClientCascade);
    }
}
